          ...
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
            <div>
              ...
              <p className="text-slate-400 text-sm">
                <span className="font-semibold text-slate-300">Owner:</span> {car.owner?.name} —{" "}
                <span className="text-indigo-400">{car.owner?.email}</span>
              </p>
            </div>

            {/* Status Badge */}
            {currentStatus && (
              <span className={`self-start px-4 py-1.5 rounded-full text-xs font-black uppercase tracking-widest border ${currentStatus.color}`}>
                {currentStatus.label}
              </span>
            )}
          </div>
